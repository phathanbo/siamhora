...
// ข้อมูลหลัก
const planetNames = ["อาทิตย์","จันทร์","อังคาร","พุธ","พฤหัส","ศุกร์","เสาร์","ราหู","เกตุ"];
const elements = ["ไฟ","ดิน","ลม","น้ำ","ดิน","น้ำ","ไฟ","ลม","วิญญาณ"];
const meanings = ["บริวาร","อายุ","เดช","ศรี","มูละ","อุตสาหะ","มนตรี","กาลกิณี"];
...
function getElementRelation(e1, e2) {
  if (e1 === e2) return {text:"เป็นกลาง", color:"text-gray-600", advice:"สถานการณ์คงที่เกี่ยวกับ"};
  const good = ((e1==="ไฟ"&&e2==="ลม") || (e1==="ลม"&&e2==="ไฟ") || (e1==="ดิน"&&e2==="น้ำ") || (e1==="น้ำ"&&e2==="ดิน"));
  return good ? 
    {text:"ส่งเสริมกัน", color:"text-green-600", advice:"พลังงานไหลลื่น ผลลัพธ์ดีเกี่ยวกับ"} : 
    {text:"หักล้างกัน", color:"text-red-600", advice:"ระวังความขัดแย้งและอุปสรรคในเรื่อง"};
}
...
function generateLongPrediction(level, mainPlanet, mainElement, meaningsList) {
  ...
  text += `<br>🔹 จุดเด่น: `;
  if (meaningsList[0].includes("บริวาร")) text += "ตัวคุณและคนรอบตัวมีบทบาทสำคัญ ";
  ...
  text += `<br>\n\n⚠️ จุดที่ควรระวัง: `;
  if (meaningsList[7].includes("กาลกิณี")) text += "ระวังอุปสรรคและความขัดข้อง โดยเฉพาะเรื่อง " + mainPlanet;
...
function calculateAll() {
  ...
  // รายปี
  html += createDetailedSection("📅 รายปี (อายุ " + age + " ปี)", yearPos, "ปี", age);

  // รายเดือน
  html += createDetailedSection("📆 รายเดือน (เดือน " + month + ")", monthPos, "เดือน", month);

  // รายสัปดาห์
  html += createDetailedSection("🗓️ รายสัปดาห์ (สัปดาห์ที่ " + weekNum + ")", weekPos, "สัปดาห์", weekNum);
